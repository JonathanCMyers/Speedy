...
public class SynStream extends ControlFrame {
	...
	public SynStream(HeaderBlock headerBlock) {
	...
	public static Frame decode(byte[] encodedBytes) throws SpeedyException {
		...
		//Decode version
		short version = decodeVersion(ByteUtility.byteSubarray(encodedBytes, index, 2));
		index += 2;
		//Decode type
		short type =(short) ByteUtility.littleEndianToUINT16(ByteUtility.byteSubarray(encodedBytes, index, 2));
		index += 2;
		//Decode flags
		byte flags = encodedBytes[index];
		index += 1;
		//Decode length
		int length = decodeLength(ByteUtility.byteSubarray(encodedBytes, index, 3));
		index += 3;
		//Decode header
		HeaderBlock headerBlock = HeaderBlock.decode(ByteUtility.byteSubarray(encodedBytes, index,encodedBytes.length - index));
		
		SynStream frame = new SynStream(headerBlock);
		return frame;
